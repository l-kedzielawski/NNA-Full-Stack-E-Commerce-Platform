      ...
      <div className="container-shell">
        <section className="mx-auto max-w-2xl rounded-2xl border border-line bg-card p-8 text-center">
          <p className="text-xs uppercase tracking-[0.16em] text-gold/60">
            {locale === "pl" ? "Zamowienie przyjete" : "Order received"}
          </p>
          <h1 className="mt-3 font-display text-4xl text-ink">
            {locale === "pl" ? "Dziekujemy za zamowienie" : "Thank you for your order"}
          </h1>
          <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-ink/55">
            {locale === "pl"
              ? "Otrzymalismy szczegoly zamowienia i wkrotce rozpoczniemy realizacje."
          ...
          {resolved.order && (
            <p className="mt-5 rounded-xl border border-gold/20 bg-gold-dim px-4 py-3 text-sm text-ink/70">
              {locale === "pl" ? "Numer zamowienia:" : "Order ID:"}{" "}
              <span className="font-semibold text-ink">{resolved.order}</span>
            </p>
          )}
